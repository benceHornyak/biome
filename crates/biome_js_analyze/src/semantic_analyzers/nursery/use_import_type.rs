...
impl Rule for UseImportType {
        ...
        match import_clause {
                ...
                match clause.specifier().ok()? {
                    AnyJsCombinedSpecifier::JsNamedImportSpecifiers(named_specifiers) => {
                        match named_import_type_fix(model, &named_specifiers) {
                            ImportTypeFix::ExtractDefaultImportType(_)
                            | ImportTypeFix::ExtractCombinedImportType
                            | ImportTypeFix::UseImportType => {
                            ...
                            ImportTypeFix::AddInlineTypeMarkers(specifiers) => {
                                if is_default_used_as_type {
                                    Some(ImportTypeFix::ExtractDefaultImportType(specifiers))
                                } else if specifiers.is_empty() {
                                    None
                                } else {
                                    Some(ImportTypeFix::AddInlineTypeMarkers(specifiers))
                                }
                            }
                        }
                    }
            ...
            AnyJsImportClause::JsImportNamedClause(clause) => {
                match named_import_type_fix(model, &clause.named_specifiers().ok()?) {
                    fix @ (ImportTypeFix::UseImportType
                    | ImportTypeFix::ExtractDefaultImportType(_)
                    | ImportTypeFix::ExtractCombinedImportType) => Some(fix),
                    ImportTypeFix::AddInlineTypeMarkers(specifiers) => (!specifiers.is_empty())
                        .then_some(ImportTypeFix::AddInlineTypeMarkers(specifiers)),
                }
            }
    ...
    fn diagnostic(ctx: &RuleContext<Self>, _: &Self::State) -> Option<RuleDiagnostic> {
        ...
        Some(RuleDiagnostic::new(
            ...
            import.range(),
            markup! {
                "Some imports are only used as a type."
            },
        ))
    }
...
pub(crate) enum ImportTypeFix {
    ...
    AddInlineTypeMarkers(Vec<AnyJsNamedImportSpecifier>),
}
...
fn named_import_type_fix(
    ...
    for specifier in specifiers.iter().filter_map(|specifier| specifier.ok()) {
    ...
    if imports_only_types && !specifiers.is_empty() {
        ImportTypeFix::UseImportType
    } else {
        ImportTypeFix::AddInlineTypeMarkers(specifiers_requiring_type_marker)
    }
}
